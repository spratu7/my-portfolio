     ...
     <div class="header">
      ...
      <div class="tips">
        ...
        <div class="dzf" v-if="detailData.trade_state === '待服务'">
          ...
          <div class="text2">请保持手机通畅,稍后将有服务专员为你联系</div>
        </div>
        <div class="dzf" v-if="detailData.trade_state === '已完成'">
          <div class="text1">服务已完成...</div>
          <div class="text2">感谢你的使用,如有售后问题请联系客服</div>
        </div>
      ...
      <!-- 支付二维码弹窗 -->
      <van-dialog :show-confirm-button="false" v-model:show="showCode" v-if="detailData.trade_state === '待支付'">
...
onMounted(async () => {
  ...
  // 直接写死配置，跳过所有复杂判断
  QRCode.toDataURL(data.data.code_url, { width:  150 })
    .then(url => {
      codeImg.value = url
    })
    .catch(err => {
      console.error('二维码失败', err)
    })
})
